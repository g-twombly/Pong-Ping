...
void lvl1_handler(char key, key_event_type_t type, double held_time,
                  game_t *game) {
...
void lvl1_check_end(game_t *game) {
  state_t *state = game_get_state(game);
...
void level1_init(game_t *game) {
  game_set_looper(game, (game_loop_fun_t)lvl1_loop);
  game_set_handler(game, (key_handler_t)lvl1_handler);
  ...
  scene_set_texture(state_get_scene(state), CALTECH_HALL_DAY_FILE);
